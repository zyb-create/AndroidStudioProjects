...
import static com.example.zyb15.lrctest.LrcViewState.Seeking;
import static com.example.zyb15.lrctest.LrcViewState.normal;

public class LrcView extends View implements ILrcView {
    private String tag = "LrcView";
    private String NoDataMessage = "加载歌词中";
    private LrcViewContext lrcContext;//上下文
    public static List<LrcRow> mRows;//行数据
    private Boolean TextSizeAutomaticMode = false;//是否文字自动适配界面
    private LrcDataBuilder lrcDataBuilder=new LrcDataBuilder();
    private int ViewWidth,ViewHeight;
    public static boolean CHANGE=false;
    private Canvas cacheCanvas=new Canvas();
    private Bitmap cacaheBitmap=null;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        cacaheBitmap=Bitmap.createBitmap(getViewWidth(),getViewHeight(),Bitmap.Config.ARGB_8888);
        cacheCanvas.setBitmap(cacaheBitmap);
    }
    ...
    private void init() {
        ...
        File file=new File("/storage/emulated/0/Music/谭维维-攀登者.txt");
        mRows=lrcDataBuilder.Build(file);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (lastShowRow != null && lastShowRow.YPosition <= getViewHeight() / 2) {
                    ...
                    if (lrcContext.CurrentState == Seeking && (moveDistance > 20 || moveDistance < -20)) {
                        seekToPosition();
                    }
    ...
    public void setFile(String path)//设置字幕路径并跟新画布
    {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (listIsEmpty(mRows)) {
            if (!TextUtils.isEmpty(NoDataMessage)) {
                cacheCanvas.drawText(NoDataMessage + "", ViewWidth / 2, ViewHeight / 2 - lrcContext.setting.MessagePaintTextSize / 2,
        ...
        if (lrcContext.CurrentState == Seeking && getLrcSetting() != null)
        {
            ...
            if (getLrcSetting().ShowTimeText) {
                cacheCanvas.drawText(TrySelectTimeText,
                        getLrcSetting().TimeTextPaddingLeft,
                        timeLineY + height / 2,
            ...
            if (getLrcSetting().ShowSelectLine) {
                //画出时间线
                cacheCanvas.drawLine(lStartX, lStartY, lStopX, lStopY, lrcContext.SelectLinePaint);
            }
            ...
            if (getLrcSetting().ShowTriangle) {
                ...
                TrianglePath.lineTo(lStopX, lStopY - TriangleWidth);
                cacheCanvas.drawPath(TrianglePath, lrcContext.SelectLinePaint);
            }
        ...
        for (int i = 0; i < mRows.size(); i++) {
            ...
            if (lrcContext.CurrentState == normal) {
                if (i == HeightLightRowPosition) {
                    drawHeightLightRow(i, lrcRow, cacheCanvas, rowX);
                } else {
                    drawNormalRow(i, lrcRow, cacheCanvas, rowX);
                }
                ...
                if (i == HeightLightRowPosition) {
                    ...
                    if (RowPositionBottom + offset >= timeLineY
                        ...
                        drawTrySelectRow(i, lrcRow, cacheCanvas, rowX);
                    } else {
                        drawNormalRow(i, lrcRow, cacheCanvas, rowX);
                    }
                }
        ...
        canvas.drawBitmap(cacaheBitmap,0,0,bmpPaint);
...
//            getLrcSetting().TrySelectRowTextSize = textHeight;
    ...
    private List<String> makeSecureLines(String text, Paint mPaint, float secureLineWidth) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            while (leftStr.length() > 0) {
                measuredNum = mPaint.breakText(leftStr, true, secureLineWidth, null);
                if (measuredNum > 0) {
    ...
    /**
     * @param automaticMoveAnimationDuration 设置歌词自动滑动时间速度
     */
